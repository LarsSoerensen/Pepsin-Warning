...
#==================================================================================
# This script will identfy if an PLGS identified sequence is in violation of the pepsin cleavage rule as identified in:
...
class Application(tk.Frame):
        ...
        with open(filelocation) as csv_file:
            ...
            for row in reader:
                if index == 0:
                    ...
                    position = int(row[26])
                    if position == 3:
                        cleavage_site =[-1, position-3, position-2, position-1, position, position+1, position+2, position+3]
                    elif position == 2:
                        cleavage_site =[-2, -1, position-2, position-1, position, position+1, position+2, position+3]
                    elif position == 1:
                        cleavage_site =[-3, -2, -1, position-1, position, position+1, position+2, position+3]
                    elif position == 0:
                        cleavage_site = [-4, -3, -2, -1, position, position+1, position+2, position+3]
                    else:
                        cleavage_site = [position-4, position-3, position-2, position-1, position, position+1, position+2, position+3] #Adds cleavage sequence of each identified peptide
    ...
    def danger_list(self):
        ...
        for key in keys:
            if self.check_dict[key][3] in danger_list_P1: #Tests the P1 position
                self.index_pop_set.add(key)
            elif self.check_dict[key][2] in danger_list_P2: #Test the P2 position
                self.index_pop_set.add(key)
            else:
                pass
        ...
        with open (filtered__location, 'w', newline = '') as csvfile:
            ...
            for key in keys:
                if key in self.index_pop_set:
                    ...
                    writer.writerow(self.main_dict[key])
...
root=tk.Tk()
root.wm_title("Pepsin Violation Checker")
